...
class MainWin(QWidget, Ui_Form):
    ...
    def setupRedirect(self):
        ...
        self.console_redirect.text_written.connect(self.appendText)
        self.original_stdout = sys.stdout
        sys.stdout = self.console_redirect

    def appendText(self, text):
        self.te_log.insertPlainText(text)
        self.te_log.moveCursor(self.te_log.textCursor().End)
    ...
    def npc_units_apply(self):
        try:
            ...
            home_hp_regen = self.box_npc_units_home_hp_regen.value()
            npc_units.change_xp_gold_and_tower_hp(path, save_path, xp_gold_mul, tower_hp_mul, home_hp_regen)
